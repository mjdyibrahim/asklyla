...
def save_review(review_info): 
    # Save the review to a database
    # You can use a database of your choice, e.g. SQLite, MySQL, MongoDB, etc.
    # Here's an example using SQLite:
    conn = sqlite3.connect("reviews.db")
    c = conn.cursor()
    c.execute("INSERT INTO reviews (restaurant_name, cuisine_type, rating, review, image_path) VALUES (?, ?, ?, ?, ?)", (review_info["restaurant_name"], review_info["cuisine_type"], review_info["rating"], review_info["review"], review_info["image_path"]))
    conn.commit()
    conn.close()

# Add a route to display all the reviews
@restaurants_bp.route("/reviews")
def show_reviews(): 
...
@restaurants_bp.route("/reviews/<int:review_id>")
...
@restaurants_bp.route("/reviews/<int:review_id>/delete", methods=["POST"])
def delete_review(review_id): 
    # Delete the review with the given ID from the database
    conn = sqlite3.connect("reviews.db")
    c = conn.cursor()
...
@restaurants_bp.route("/reviews/<int:review_id>/update", methods=["POST"])
...
@restaurants_bp.route("/restaurant_types")
def get_restaurant_types(): 
    # Retrieve all the restaurant types from the database
    conn = sqlite3.connect("reviews.db")
    c = conn.cursor()
    c.execute("SELECT DISTINCT cuisine_type FROM reviews")
    restaurant_types = [row[0] for row in c.fetchall()]
    conn.close()

    return jsonify({"restaurant_types": restaurant_types})

# Add a route to get all the reviews for a specific restaurant type
# @app.route("/reviews/string:restaurant_type")
@restaurants_bp.route("/reviews/<string:restaurant_type>")
def get_reviews_by_restaurant_type(restaurant_type): 
    # Retrieve all the reviews for the given restaurant type from the database
    conn = sqlite3.connect("reviews.db")
    c = conn.cursor()
    c.execute("SELECT * FROM reviews WHERE cuisine_type=?", (restaurant_type,))
    reviews = c.fetchall()
    conn.close()
...
# @app.route("/average_rating/string:restaurant_type")
@restaurants_bp.route("/average_rating/<string:restaurant_type>")
def get_average_rating_by_restaurant_type(restaurant_type): 
    # Retrieve the average rating for the given restaurant type from the database
    conn = sqlite3.connect("reviews.db")
    c = conn.cursor()
    c.execute("SELECT AVG(rating) FROM reviews WHERE cuisine_type=?", (restaurant_type,))
